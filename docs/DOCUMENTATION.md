...
- For CQRS + SSE: Indicator resets when _command completes_, not when _query renders_
...
- **Verification**: after adding or editing components, run `pnpm --filter honostar-demo build:css` and `pnpm --filter honostar-demo typecheck`.
...
**Core Framework Structure** (`packages/core/src/`)

- `server/router/` - File-based routing with compile-time manifest generation (runtime-agnostic)
- `server/sse/` - SSE event bus, responders, formatters, and middleware
- `server/page.ts` - Type-safe page and handler definitions
- `server/route.ts` - Type-safe route helpers with parameter extraction
- `server/renderer.tsx` - Server-side JSX renderer with theme, CSP, and nonce support
- `common/theme.ts` + `client/theme.ts` - Server/client theme system
- `server/security/` - CSRF + topic signing
- `server/middleware.ts` - Factory for creating app middleware
...
19. **Lint + typecheck** (`pnpm lint`, `pnpm typecheck`) succeed locally; include results in your summary if requested.
