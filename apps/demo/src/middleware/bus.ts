import { factory, MemoryBus, type PubSubBus } from '@honostar/core/server'
...
const createBus = async (): Promise<PubSubBus> => {
  ...
  if (natsUrl) {
    try {
      const { NatsBus } = await import('@honostar/core/server')
  ...
  if (redisUrl) {
    try {
      const { RedisBus } = await import('@honostar/core/server')
