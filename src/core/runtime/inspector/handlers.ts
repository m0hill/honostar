...
function normalizeShortcut(shortcut: string): NormalizedShortcut {
  ...
  tokens.forEach(token => {
    ...
    switch (upper) {
      case 'SHIFT':
        normalized.requireShift = true
        break
      case 'ALT':
      case 'OPTION':
        normalized.requireAlt = true
        break
      case 'CTRL':
      case 'CONTROL':
      case 'CMDORCTRL':
      case 'CTRLORMETA':
        normalized.requireCtrlOrMeta = true
        break
      case 'META':
      case 'CMD':
      case 'COMMAND':
        normalized.requireMeta = true
        break
      default:
  ...
  if (!normalized.key && tokens.length > 0) {
    normalized.key = tokens[tokens.length - 1]!.toUpperCase()
  }

  return normalized
}

export function setupKeyboardShortcut(keyboardShortcut: string, onToggle: () => void): () => void {
  ...
  const listener = (evt: KeyboardEvent) => {
    ...
    if (shortcut.requireShift && !evt.shiftKey) return
    if (shortcut.requireAlt && !evt.altKey) return

    if (shortcut.requireCtrlOrMeta) {
      if (!(evt.ctrlKey || evt.metaKey)) return
    } else {
      if (shortcut.requireCtrl && !evt.ctrlKey) return
      if (shortcut.requireMeta && !evt.metaKey) return
    }
